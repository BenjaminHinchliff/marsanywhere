...
\subsection{Loss Function Ground Truth Masking}
...
imagery that is truly present in the imagery. This is done by shareholding for
...
\subsection{Mutual Information Masking}

Additional experiments were conducted with masking loss function on the intersection between the data present in both the conditioning image and the true ground imagery. We refer to this as mutual information masking since we are masking to only train the model on the mutually included information between the views. This was done by taking the bitwise and of a mask of both the ground and prior imagery, then applying that to the predicted imagery and the ground imagery for loss calculation, as shown in figure~\ref{fig:mi-masking-ex}.

\begin{figure}

\includegraphics[width=\linewidth]{full-mask-ex.jpg}
\caption{Example pipeline for mutual information masking the loss, with MSE loss as an example.}
