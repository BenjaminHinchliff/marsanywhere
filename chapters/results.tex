...
While the initial results are promising, our model exhibits a number of
artifacts that degrade the geometrical accuracy and photorealism of the image.

A major known artifact is that the model attempts to reconstruct the
Perseverance rover within the imagery. This fact alone may have been why full
image masking was met with limited success, as the rover often occluded the
overlapping portion of the image. This leads the model to predict the rover more
often than not, which is not our goal in producing the imagery. This is then
exacerbated by the image split issues, causing the rover to occlude almost all
of the image in many of the fully masked cases.

In cases where the model does predict terrain though, it often performs quite
well. For instance, in prediction of 0533 and 0627, it was able to accurately
infer the shape of terrain and reconstruct it, as shown in
figure~\ref{fig:no-mask-good}. However, in other cases, it struggles to
reproduce the large scale features or hallucinate non-existent textures, such as
in figure~\ref{fig:no-mask-bad}.
